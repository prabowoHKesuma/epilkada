<x-guest-layout>
    <div class="max-w-3xl mx-auto mt-6 p-6">
        ...
        <form action="{{ route('voting.submit.remote') }}" method="POST" id="voteForm">
            ...
            <div id="statusBox" class="mb-4 p-3 rounded bg-gray-100 text-gray-600 text-center">Belum ada kandidat dipilih.</div>

            <button type="button" id="btnSimpan" class="w-full py-3 bg-indigo-600 text-white rounded font-semibold mb-3" disabled>Simpan Pilihan</button>
            <button type="submit" id="btnKirim" class="w-full py-3 bg-green-600 text-white rounded font-semibold text-lg hidden">Konfirmasi & Kirim Suara</button>
        </form>
    </div>
    ...
    <script>
        ...
        cards.forEach(card => {
            card.addEventListener('click', () => {
                ...
                statusBox.className = 'mb-4 p-3 rounded bg-blue-100 text-blue-800 text-center';
        ...
        btnSimpan.addEventListener('click', () => {
            ...
            statusBox.textContent = 'Pilihan tersimpan: ' + selectedCard.dataset.name + '. Periksa lagi, lalu tekan tombol hijau.';
            statusBox.className = 'mb-4 p-3 rounded bg-green-100 text-green-800 text-center';
